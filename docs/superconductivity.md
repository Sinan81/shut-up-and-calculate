...
It's instructive to look at spectra along a cut where d-wave gap significantly modifies the electron band near Fermi level:

![](/images/tetra/sc_dwave_spectra.png)

These plots can be generated as follows:
```python
from tba import * ; x = TetraSingleBandSC(filling=0.5,mu=-0.19718994140625) ;
x.plot_bands_along_sym_cuts(plot_Emin=-2,plot_Emax=2)
x.spectra.plot_spectra_along_kx_cut(Emin=-0.5,Emax=0.5,kmin=-0.3*pi,kmax=0.3*pi,iorb=0,kx=1.0*pi)
```
